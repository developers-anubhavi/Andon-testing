...
import Settings from '../MK1/Settings';
...
export default function App() {
  ...
  const logoutUser = useCallback(() => {
    if (username) {
      fetch("http://192.168.0.24:4002/api/logout", {
        method: "POST",
  ...
  useEffect(() => {
    const handleUnload = () => {
      if (username) {
        ...
        navigator.sendBeacon("http://192.168.0.24:4002/api/logout", blob);
      }
...
const handleLogin = async () => {
  ...
  try {
    const response = await fetch(
      "http://192.168.0.24:4002/api/logincredentials",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
