...
export default function UserDetails({ logout, username: loggedInUsername }: UserDetailsProps) {
  ...
  const refreshUsers = () => {
    fetch("http://192.168.0.24:4002/api/bs_users")
      .then((res) => res.json())
  ...
  const addUser = () => {
    ...
    fetch("http://192.168.0.24:4002/api/users/bs_add", {
      method: "POST",
  ...
  const deleteUser = (id: number) => {
    fetch(`http://192.168.0.24:4002/api/users/bs_delete/${id}`, {
      method: "DELETE",
    }).then(() => {
      refreshUsers();
