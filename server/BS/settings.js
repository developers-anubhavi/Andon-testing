...
/** Generic number file reader with padding */
...
router.post('/bs_update-table', (req, res) => {
  try {
...
function broadcastTableData() {
  ...
  wss.clients.forEach(client => {
    if (client.readyState === WebSocket.OPEN) client.send(JSON.stringify({ type: 'tableData', data }));
  });
}
...
function broadcastShiftTimes() {
  ...
  wss.clients.forEach(client => {
    if (client.readyState === WebSocket.OPEN) client.send(JSON.stringify({ type: 'shiftTimes', data }));
  });
}
...
function setupWebSocket(server) {
  ...
  // Watch all files and broadcast changes
